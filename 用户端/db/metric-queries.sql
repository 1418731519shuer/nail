...
LEFT JOIN (
  SELECT
    store_id,
    COUNT(*) FILTER (WHERE status IN ('paid', 'completed')) AS order_count,
    COALESCE(SUM(amount) FILTER (WHERE status IN ('paid', 'completed')), 0) AS revenue_amount,
    COUNT(*) FILTER (WHERE source = 'ai_try_on' AND status IN ('paid', 'completed')) AS ai_order_count,
    COALESCE(SUM(amount) FILTER (WHERE source = 'ai_try_on' AND status IN ('paid', 'completed')), 0) AS ai_revenue_amount
  FROM orders
  WHERE store_id = :store_id
