...
let currentPageId = "catalog";
let handModelPreviewUrl = "";
let styleVisibilityObserver = null;
let latestPersonalizationText = "";
let currentRecommendConfigVersion = "";
let recommendSyncTimer = null;
const recommendSessionId = makeLocalId("recommend_session");
...
async function loadXhsDataset() {
  try {
    const response = await fetch("/api/xhs-style-dataset");
    if (!response.ok) throw new Error(`dataset ${response.status}`);
    const payload = await response.json();
    if (Array.isArray(payload.styles) && payload.styles.length) {
      nails = payload.styles;
    }
    xhsFilterGroups = payload.filterGroups || {};
  } catch (error) {
    console.warn("[xhs-style-dataset] fallback to bundled demo styles", error);
    nails = [...nailStyleDB];
    xhsFilterGroups = {
      分类: uniqueValues(nails, "primaryTag")
    };
  }
}
...
function currentFilteredNails() {
  return nails.filter((item) => {
    return Object.entries(filterState).every(([group, value]) => {
      if (value === "全部") return true;
      return (item.tagGroups?.[group] || []).includes(value);
    });
  });
}

const defaultCatalogRecommendSlots = [
  { slotType: "hero_full_1", userLabel: "今日精选", reason: "hot_conversion", category: "hot" },
  { slotType: "hero_full_2", userLabel: "店内常做款", reason: "hot_conversion", category: "hot" },
  { slotType: "hero_full_3", userLabel: "小众但好看", reason: "potential_activation", category: "potential" },
  { slotType: "hero_full_4", userLabel: "适合你试试", reason: "personalized_tag_match", category: "personalized" },
  { slotType: "peek_half_1", userLabel: "正在流行", reason: "hot_continuation", category: "hot" },
  { slotType: "peek_half_2", userLabel: "新款灵感", reason: "potential_second_test", category: "potential" },
  { slotType: "peek_half_3", userLabel: "冷门观察", reason: "cold_watch", category: "cold" },
  { slotType: "peek_half_4", userLabel: "换个风格", reason: "personalized_tag_match", category: "personalized" }
];

let catalogRecommendSlots = defaultCatalogRecommendSlots.map((slot) => ({ ...slot }));
...
function currentIntentText() {
  const activeFilters = Object.entries(filterState)
    ...
    .join(" ");
  return `${latestPersonalizationText} ${activeFilters}`.trim();
}

function scorePersonalizedItem(item, intentText) {
  const haystack = [
    item.name,
    item.primaryTag,
    item.secondaryTag,
...
function rankPoolForCategory(pool, category, intentText) {
  const ranked = [...pool].sort((left, right) => {
    if (category === "hot") return (right.hotScore || 0) - (left.hotScore || 0);
    if (category === "potential") {
      const leftPotential = left.recommendBucket === "potential" ? 1 : 0;
      const rightPotential = right.recommendBucket === "potential" ? 1 : 0;
      return rightPotential - leftPotential || (right.hotScore || 0) - (left.hotScore || 0);
    }
    if (category === "cold") return (left.hotScore || 0) - (right.hotScore || 0);
    return scorePersonalizedItem(right, intentText) - scorePersonalizedItem(left, intentText) || (right.hotScore || 0) - (left.hotScore || 0);
  });

  if (category === "hot") {
    return uniqueById(ranked.filter((item) => item.recommendBucket === "hot" || item.recommendBucket === "stable").concat(ranked));
  }
  if (category === "potential") {
    return uniqueById(ranked.filter((item) => item.recommendBucket === "potential" || item.recommendBucket === "new").concat(ranked));
  }
  if (category === "cold") {
    return uniqueById(ranked.filter((item) => item.recommendBucket !== "hot").concat(ranked));
  }
...
function pickRecommendedItem(pool, slot, intentText) {
  const ranked = rankPoolForCategory(pool, slot.category || "hot", intentText);
  return ranked[0] || pool[0];
}

function buildCatalogRecommendationFlow(pool) {
  if (!pool.length) return [];
  const remaining = uniqueById(pool);
  const intentText = currentIntentText();
  const result = [];
  let cycleIndex = 0;

  while (remaining.length) {
    const cycleStart = result.length;
    for (let slotIndex = 0; slotIndex < catalogRecommendSlots.length && remaining.length; slotIndex += 1) {
      const slot = catalogRecommendSlots[slotIndex];
      const item = pickRecommendedItem(remaining, slot, intentText);
      if (!item) break;
      const remainingIndex = remaining.findIndex((candidate) => candidate.id === item.id);
      ...
      result.push({
        ...
        recommendSlot: {
          ...
          globalPosition: result.length + 1,
          visibleMode: slotIndex < 4 ? "完整露出" : "半露出"
        }
      });
    }

    if (result.length === cycleStart) break;
    cycleIndex += 1;
  }
...
function normalizeRemoteRecommendSlots(slots) {
  if (!Array.isArray(slots) || !slots.length) {
    return defaultCatalogRecommendSlots.map((slot) => ({ ...slot }));
  }
  return slots.slice(0, 8).map((slot, index) => ({
    slotType: index < 4 ? `hero_full_${index + 1}` : `peek_half_${index - 3}`,
    userLabel: slot.strategyLabel || slot.slotName || defaultCatalogRecommendSlots[index].userLabel,
    reason: slot.reason || defaultCatalogRecommendSlots[index].reason,
    category: slot.category || defaultCatalogRecommendSlots[index].category
  }));
}

function applyRemoteRecommendConfig(config) {
  if (!config) return;
  catalogRecommendSlots = normalizeRemoteRecommendSlots(config.slots);
  currentRecommendConfigVersion = config.versionId || "";
  updateCatalog();
}

async function syncRecommendActivity(overrides = {}) {
  try {
    await fetch("/api/recommend-sync-activity", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sessionId: recommendSessionId,
        sessionOpen: overrides.sessionOpen === false ? false : true,
    ...
    // Ignore sync heartbeat failures on the client.
  }
}

async function pullRecommendConfig() {
  try {
    const response = await fetch("/api/recommend-sync-state");
    const state = await response.json();
    if (!response.ok) return;

    if (state.publishedConfig && state.publishedConfig.versionId !== currentRecommendConfigVersion) {
      applyRemoteRecommendConfig(state.publishedConfig);
    }
  } catch {
    // Keep local recommendation fallback when sync API is unavailable.
  }
}

function renderFilters() {
  const orderedGroups = ["季节", "风格", "款式", "甲型"];
  orderedGroups.forEach((group) => {
    const candidates = ["全部", ...(xhsFilterGroups[group] || [])];
    if (!candidates.includes(filterState[group])) {
      filterState[group] = "全部";
    }
  });

  const renderGroupSection = (group) => `
    <section class="filter-group-block">
      <header>${group}</header>
      <div class="filter-pills group-row">
        ${["全部", ...(xhsFilterGroups[group] || [])].map((value) => `
          <button class="pill ${value === filterState[group] ? "active" : ""}" data-filter-group="${group}" data-filter-value="${value}">
            ${value}
          </button>
        `).join("")}
      </div>
  ...
  primaryFilters.innerHTML = orderedGroups.slice(0, 2).map(renderGroupSection).join("");
  secondaryFilters.innerHTML = orderedGroups.slice(2).map(renderGroupSection).join("");
}
...
function renderCards(items = currentFilteredNails()) {
  ...
  grid.innerHTML = items.map((item, index) => `
    <article
      class="nail-card ${item.recommendSlot ? "strategy-slot" : "browse-slot"} ${item.recommendSlot?.position > 4 ? "peek-slot" : ""}"
    ...
    >
      <button class="nail-thumb ${item.image ? "has-image" : ""}" style="--thumb: ${item.thumb}; --nail: ${item.nail}; --accent: ${item.accent}" data-id="${item.id}" aria-label="试戴${item.name}">
        ${item.image ? `<img src="${item.image}" alt="${item.name}" onerror="this.closest('.nail-thumb')?.classList.remove('has-image'); this.remove();" />` : ""}
      </button>
      <div class="card-body">
        ...
        ${item.marketingTitle ? `<p class="marketing-title">${escapeHTML(item.marketingTitle)}</p>` : ""}
...
function renderReviewList(item, limit = 3) {
  const reviewDetails = Array.isArray(item.reviewDetails) && item.reviewDetails.length
    ? item.reviewDetails
    : (item.reviews || []).map((text) => ({ text, likes: 0 }));
  return reviewDetails.slice(0, limit).map((review, index) => `
    <div class="review">
      <div class="review-head">
        <strong>${maskReviewUserName(item, review, index)}</strong>
        <span>${Number(review.likes || 0)} 赞</span>
      </div>
      <div class="review-copy">“${escapeHTML(review.text || "")}”</div>
    </div>
  `).join("");
}

function maskReviewUserName(item, review, index) {
  if (review?.maskedUserName) return escapeHTML(review.maskedUserName);
  const sourceName = String(review?.userName || "").trim();
  if (!sourceName) return `用户${index + 1}`;
  const chars = Array.from(sourceName);
  if (chars.length === 1) return `${chars[0]}*`;
  const middleIndex = Math.floor(chars.length / 2);
  return escapeHTML(chars.map((char, charIndex) => (charIndex === middleIndex ? "*" : char)).join(""));
}
...
function updateCatalog() {
  ...
  const activeFilters = Object.entries(filterState)
    .filter(([, value]) => value !== "全部")
    .map(([group, value]) => `${group}：${value}`);
  const groupCount = Math.ceil(filteredItems.length / 8);
  filterStatus.textContent = `当前筛选 ${activeFilters.length ? activeFilters.join(" / ") : "全部标签组合"}，共 ${filteredItems.length} 款；每 8 款循环应用一组推荐策略，共 ${groupCount} 组`;
}
...
function openTryOn(id) {
  ...
  dialogTitle.textContent = `${item.marketingTitle || item.name}试戴`;
...
function renderMiniPicks(picks = nails.slice(0, 3)) {
  const pinnedIds = new Set(picks.map((item) => item.id));
  const fallbackPool = [...currentFilteredNails(), ...nails]
    .filter((item, index, arr) => item && arr.findIndex((candidate) => candidate.id === item.id) === index)
    .sort((a, b) => {
  ...
  const feed = fallbackPool;

  recommendResults.innerHTML = feed.map((item, index) => `
    <article class="mini-pick">
      <img src="${item.image}" alt="${item.name}" />
      <div>
        <h3>${item.name}</h3>
        ${item.marketingTitle ? `<strong class="mini-pick-title">${escapeHTML(item.marketingTitle)}</strong>` : ""}
        <p>${item.primaryTag}/${item.secondaryTag} · ${item.rating}/5.0 · ${item.likes} 小红书点赞</p>
        ${item.reviewDetails?.[0] ? `<small>${maskReviewUserName(item, item.reviewDetails[0], 0)} · ${Number(item.reviewDetails[0].likes || 0)} 赞：${escapeHTML(item.reviewDetails[0].text || "")}</small>` : item.reviews?.[0] ? `<small>用户评价：${escapeHTML(item.reviews[0])}</small>` : ""}
        ${index < picks.length ? `<span class="mini-pick-badge">AI 优先推荐</span>` : ""}
      </div>
    </article>
  `).join("");
}
      ...
      ${picks.map((item) => `
        <article class="chat-rec-card">
          ...
          <div>
            ...
            ${item.marketingTitle ? `<small>${escapeHTML(item.marketingTitle)}</small>` : ""}
...
document.addEventListener("click", (event) => {
  ...
  if (filterButton) {
    ...
    if (group) filterState[group] = value;
    updateCatalog();
  }
  if (pageTarget) {
...
chatForm?.addEventListener("submit", (event) => {
  ...
  if (!text) return;
  latestPersonalizationText = text;
...
document.querySelectorAll("[data-prompt]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    latestPersonalizationText = button.dataset.prompt || "";
    chatInput.focus();
  });
});

window.addEventListener("focus", syncRecommendActivity);
window.addEventListener("blur", syncRecommendActivity);
...
window.addEventListener("pagehide", () => {
  try {
    fetch("/api/recommend-sync-activity", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sessionId: recommendSessionId,
  ...
  if (recommendSyncTimer) {
    window.clearInterval(recommendSyncTimer);
    recommendSyncTimer = null;
  }
});
...
async function bootstrapApp() {
  ...
  syncRecommendActivity();
  pullRecommendConfig();
  recommendSyncTimer = window.setInterval(() => {
    syncRecommendActivity();
    pullRecommendConfig();
  }, 15000);
}
