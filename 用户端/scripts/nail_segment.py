...
def main():
    ...
    for item in detections:
        ...
        if assignment and assignment.get("assigned"):
            label = f"{item['id']} {assignment['image_side_label']}-{assignment['finger']}"
        else:
            label = f"{item['id']} unknown"

        color = (68, 154, 139)
        draw_transparent_polygon(image, polygon, color)
        cv2.rectangle(image, (int(x1), int(y1)), (int(x2), int(y2)), (120, 120, 120), 1)
        draw_oriented_box(image, oriented_box, (217, 79, 85))
        cv2.putText(image, label, (int(x1), max(18, int(y1) - 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (34, 31, 38), 2)
        cv2.circle(image, (int(center[0]), int(center[1])), 4, (196, 154, 77), -1)
